...
- **Inline vs standalone auto-routing**: when `icon` is unset AND `hideTrailingIcon=true`, ExternalLink renders as a plain `<Text onPress>` (respects the parent text baseline). Otherwise it renders as an `<XStack>` (the standalone layout with fixed-size icon wrappers). Consumers don't pick the mode — the props naturally route it. This fix was retrofitted after shipping — nesting `<View>` inside `<Text>` in RN causes the child to float above the baseline, which read as broken in the example page's "Inline in body copy" demo.
