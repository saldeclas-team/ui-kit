 ...
 * by default so we import `.android` explicitly to exercise this
 * variant.
 */
...
import { BottomSheetBody } from "./bottom-sheet-body.android";
...
describe("BottomSheetBody.android", () => {
  beforeEach(() => {
  ...
  it("forwards palette.background as backgroundStyle.backgroundColor", async () => {
    await render(
      <BottomSheetBody {...DEFAULT_PROPS} chromeColors={{ background: "#123456" }}>
        <></>
      </BottomSheetBody>
    );
    expect(screen.getByTestId("bs-sheet").props["data-bg"]).toBe("#123456");
  });
  ...
  it("all ref methods forward to the native ref", async () => {
    ...
    expect(fakeNativeRef.present).toHaveBeenCalledWith(0);
